...
use Grace\DBAL\PdoPgsql\Connection;
use Grace\DBAL\Exception\ConnectionException;
use Grace\DBAL\Exception\QueryException;

class PdoPgsqlConnectionTest extends ConnectionTestAbstract
{
    /** @var Connection */
    ...
    protected function setUp()
    {
        ...
        if (!$this->connection->isPhpEnvironmentSupported()) {
            $this->markTestSkipped('No PDO Pgsql support in php');
        }
    ...
    {
        $r = $this->connection->replacePlaceholders("SELECT ?q, '?e', \"?p\", ?f, ?F, ?l, ?v, ?i, ?q:named_pl: FROM DUAL", array(
            '\'quoted\'',
            '\'escaped\'',
            '\'plain\'',
            'test',
